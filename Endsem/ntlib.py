...
def pde(l_x,l_t,N_x,N_t):
    ...
    plt.plot(X, V0)
    alpha  = (h_t/(h_x**2))
    print(alpha,"is the value of alpha")
    V1 = np.zeros(N_x+1)
...
def Forward_euler(x0,y0,N,x1,f):
    h=(x1-x0)/N
    x = x0
    print(x)
    y = y0
...
def shooting(d2ydx2, dydx, x0, y0, xf, yf, z_g1, z_g2, st, tol):
    x, y, z = RK4_S(d2ydx2, dydx, x0, y0, z_g1, xf, st)
    yn = y[-1]
    if abs(yn - yf) > tol:
        if yn < yf:
            zl = z_g1
            yl = yn
            x, y, z = RK4_S(d2ydx2, dydx, x0, y0, z_g2, xf, st)
            yn = y[-1]
            if yn > yf:
                zh = z_g2
                yh = yn
                z = Lag_Int(zh, zl, yh, yl, yf)
                x, y, z = RK4_S(d2ydx2, dydx, x0, y0, z, xf, st)
                return x, y, z
            else:
...
def RK4_S(d2ydx2, dydx, x0, y0, z0, xf, st):
